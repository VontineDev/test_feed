...
> v1.0.1.0부터 **조합전략 텔레그램 연동 + daily_ohlcv 워밍 잡**이 추가되었습니다. `/backtest compose <strategy> <start> <end> [market]` — AND-1 \| AND-2 \| SCORE-1 \| FUNNEL-1 \| ALL. ALL 선택 시 4전략 순차 실행 후 비교표(샤프28d 내림차순) 전송. `jobs/ohlcv_warm.py` — KRX OpenAPI 기반 daily_ohlcv 히스토리 백필(`--start 2025-01-02`) + 일배치(평일 18:30 KST). KOSPI/KOSDAQ → 'KR' 정규화(VARCHAR(4) 제약 수정). 초기 백필 완료: 71,341행 / 1,442 distinct dates (2020-12 ~ 2026-06).
> v1.0.0.0부터 **Tier-1 조합전략 백테스트 시스템**이 추가되었습니다. `analysis/strategy_compose.py` — 신호 합성 레이어(load_signal_frame·derive_flags·and_gate·composite_score·funnel). `backtest_engine.py`에 `compose` 모드 추가 — JOIN+백필 아키텍처로 라이브 스크리너와 백테스트가 동일 precompute 테이블 사용. Tier-1 전략 4종: AND-1(이치모쿠 ∩ Stage2+ ∩ 수급 비매도, 샤프 1.75) / AND-2(AND-1 ∩ 거래량 중앙값 이상) / SCORE-1(z-score 가중합 top-20) / FUNNEL-1(수급 스크린 → 4주 내 이치모쿠 트리거, 샤프 0.74·신호 2,009건). 히스토리 백필: `jobs/stage_backfill.py`(stage_classifications 전 구간), `jobs/screener_backfill.py`(fetch-once 최적화 — 2,768종목 1회 다운로드, 69주 4,144건). CLI: `scripts/run_compose.py --strategy ALL --start 2025-01-01 --end 2026-06-14`. 단위/통합/패리티 테스트 63개 추가.
