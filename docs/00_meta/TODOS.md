...
3. **B그룹 (10종목, 11행 — 가격 단서 전혀 없음)** — `003490.KS`/`008470.KQ`/
   ...
   스냅샷 — 실행 직전 재조회 권장).
4. **C그룹 (3건, 별개 드리프트)** — `000650.KS`/`001080.KS`/`003480.KS`. 2026-05-16에
   이미 `status='closed', exit_type='reconciled_hardstop'`으로 DB상 "정리 완료"
   처리됐는데 브로커엔 여전히 실보유(각 9/1987/1706주)가 남아있음 — 08-05
   종목코드 접두사 버그와 무관한 **더 오래된 별개 사례**로 보임(왜 그 시점 hardstop
   reconciliation이 브로커에 실제로 반영 안 됐는지 원인 미상). 이번 세션에서
   손대지 않음 — 별도 investigate 필요.
