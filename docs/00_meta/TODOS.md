...
**How to apply:** `python scripts/daily_ohlcv_backfill.py --start 2022-01-01`
(백그라운드, ~68분 소요) 완료 — 종목당 2022-01-03~2026-08-07까지 daily
단위로 채워짐(2,798,528행, 3,053종목). 정기 유지보수로 스케줄러에 주기
실행(예: 주 1회, 최근 며칠치만) 등록하면 재발 방지에 도움 — 아직 미등록
(수동 백필만 완료).

**정정 — 캐시 레이어 자체도 버그였음(백필 직후 재확인, 2026-08-10):**
위에서 "캐시 레이어 자체는 버그 없이 정상 동작"이라고 썼는데 **틀렸음** —
백필 직후에도 캐시가 계속 히트 0으로 잡혀서 재조사한 결과 `core/
ohlcv_cache.py`에 별개의 버그 2건이 더 있었다:
1. `_get_coverage()`의 SQL이 `WHERE date BETWEEN start AND end`로 걸려있어
   `MIN(date)`가 정의상 `start`보다 절대 작아질 수 없음 — 아무리 백필해도
   "start 이전까지 데이터 있음"을 영원히 증명 못 하는 구조적 버그.
2. 캘린더 "어제"(effective_end)와 "마지막 거래일"을 혼동 — 주말/공휴일
   다음 날엔 데이터가 완전히 최신이어도 항상 미스로 잡힘.

수정(`core/ohlcv_cache.py`): (1)의 SQL 하한 제거, (2)에 4일 슬랙 허용,
분류 로직을 `_classify_coverage()` 순수 함수로 추출해 회귀 테스트 8건
추가(`tests/test_ohlcv_cache_coverage.py`). 부수 발견: 캐시가 실제로
히트하기 시작하자 `_load_df()`가 종목마다 DB 커넥션을 새로 열어 순차
로드하는 게 새 병목으로 드러남(종목당 ~400ms, 1583종목이면 10분+) —
`_load_df_bulk()`(단일 쿼리 + 커넥션 1개 재사용)로 교체.

**최종 검증**(FUNNEL-1, 2025-01-02~2026-08-06, 1583종목): 캐시 히트
0→1,455(92%), 신호수 정상(4,643건), 전체 실행시간 96초(수정 전 커넥션
병목 상태 10분+ 대비, yfinance 경로였던 최초 상태 3분+ 대비 모두 단축).
전체 스위트 971 passed.
...
**FUNNEL-1/SCORE-1과 동일 방법론 정밀 비교 — ✅ 완료(2026-08-10):** 기존 비교는
FUNNEL-1/SCORE-1의 승률28d(고정 28일 뒤 종가 기준)와 SCENARIO2의 실현승률(RSI/
손절 청산 기준)을 서로 다른 방식으로 비교하고 있었음 — `scripts/run_compose.py
--strategy FUNNEL-1|SCORE-1 --start 2025-01-02 --end 2026-08-06`로 두 전략의
**매도신호 승률/평균수익**(`win_rate_sell`/`avg_return_sell`, 즉 SCENARIO2와 동일하게
실제 청산 시점 기준 실현 성과)을 같은 기간으로 재산출해 정확히 맞춤:

| 전략 | 신호수 | 실현 승률 | 실현 평균수익 | MDD |
|------|--------|-----------|----------------|-----|
| FUNNEL-1 | 4,643 | **64.1%** | +9.7% | -35.9% |
| SCORE-1 | 1,658 | **70.6%** | +9.2% | -39.5% |
| 2안 (문서 원안, PER≤15) | 100 | 43.0% | +2.9% | 미측정 |
| 2안 (보수적 최적화, PER≤18+청산완화) | 129 | 50.4% | +9.47% | 미측정 |
| 2안 (이론적 상한, PBR단독+청산완화, 과최적화 위험) | 116 | 55.2% | **+16.82%** | 미측정 |

**결론 재확인 + 근거 명확화**: 승률은 FUNNEL-1/SCORE-1이 확실히 우위(2안 최적화
상한 55.2%조차 SCORE-1 70.6%보다 15%p 낮음, 표본도 100~129건 vs 1,658~4,643건으로
자릿수 차이). 반면 **건당 평균수익은 생각보다 근접** — 보수적 2안(+9.47%)은
FUNNEL-1(+9.7%)·SCORE-1(+9.2%)과 사실상 동급이고, 상한(PBR, +16.82%)만 두드러지게
높지만 이는 필터·청산을 순차 최적화해 쌓은 과최적화 위험 큰 수치. 즉 차이는
"건당 수익"이 아니라 **"얼마나 자주 맞히냐(승률)"와 표본 신뢰도**에서 남 —
기존 "여전히 열위" 결론은 유지되지만, 근거가 방법론이 다른 두 지표의 비교에서
동일 방법론 정밀 비교로 강화됨. SCENARIO2 쪽 MDD는 `quant_signals.py`
파이프라인에 아직 미구현이라 비교 불가(향후 필요 시 추가 가능).
...
시장귀인 분석 2026-08-08~08-10 완료, FUNNEL-1/SCORE-1 동일방법론 정밀비교 2026-08-10
완료 — 항목 종결. 상세 근거·비교표는 `project_technicalquant_backtest` 메모리(1~8단계) 참고.
...
2. **A그룹 (7건, 신호가 단서 있음)** — `079160.KS`/`383220.KS`/`005935.KS`/
   ...
   Open 포지션 17→24건.
3. **B그룹 (10종목, 11행 — 가격 단서 전혀 없음) — ✅ 9/10 완료(2026-08-11 장중)**
   `003490.KS`/`008470.KQ`/`017650.KQ`/`028670.KS`(중복 2행)/`052460.KQ`/
   `138040.KS`/`252990.KQ`/`278470.KS`/`316140.KS`/`001540.KQ`. DB 라벨은
   `exit_type='reconciled_no_data'`로 정정 완료(실제로는 체결됐으나 진입가
   복구 불가함을 명시). 2026-08-10 16:09 KST 매도 시도는 `RC4058(모의투자
   장종료)`로 전건 거부됐었는데, **2026-08-11 14:49 KST 장중에 재시도해
   9종목 완전 체결 완료** — `exit_type='reconciled_sold'`, 실제 주문번호
   기록 완료(003490.KS/138040.KS/252990.KQ/278470.KS/316140.KS/028670.KS/
   017650.KQ/052460.KQ/001540.KQ).

   **`008470.KQ`만 미해결** — 2,596주 매도주문(`0148583`) 접수 후 **12분간
   전혀 안 움직임**(다른 9종목은 전부 5분 이내 완전체결). 실제 일별 거래량은
   충분(최근 하루 2.2만~12만주)해서 종목 자체 유동성 문제는 아니고, 모의투자
   서버의 시뮬레이션 매칭엔진 쪽 불안정으로 추정. 원주문이 살아있는 채로
   남아있으면 그 수량이 계속 잠겨 재주문도 "매도가능수량 부족"으로 거부되는
   걸 확인 — `KiwoomPaperTrader.cancel_order()`(kt10003, 이번에 신규 구현)로
   원주문 취소 후 재주문(`0155296`, 2,440주 — 취소 시점에 이미 156주는
   체결 확인됨)했으나 **재주문도 부분체결(2,440→2,422, 18주만 체결)에
   그친 채 15:17 KST 기준 장마감 임박으로 중단**. 다음 거래일 장중에
   `pt.get_position_qty('008470.KQ')`로 잔여수량 재확인 후 재시도 필요
