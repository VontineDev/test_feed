...
PC와 모바일 공통으로 동일한 탭 이름·순서를 사용합니다.
...
집 밖에서 폰으로 대시보드에 접근하고 싶다면 포트포워딩 + DDNS를 설정합니다.

**1. 포트포워딩**: 공유기 관리 페이지에서 외부 포트 8000 → 서버 IP:8000 포워딩 추가.

**2. DuckDNS DDNS** (선택 — 공인 IP가 자주 바뀌는 경우):
```powershell
# duckdns_update.bat 실행 (프로젝트 루트)
# .env에 아래 항목 추가 후 작업 스케줄러에 등록
DUCK_DNS_DOMAIN=yoursubdomain   # vtrading 예시
...
키움 모의투자 현재 포지션 현황을 조회합니다. 모델별(stage/kosdaq/cross/ichimoku/compose-funnel1/compose-and1/compose-score1)로 open·pending 포지션을 표시하며, 실시간 가격과 미실현 수익률을 보여줍니다.
...
7개 모델 각각의 진입 신호 소스와 검증된 청산 파라미터입니다.
...
| `compose-funnel1` | 주간 FUNNEL-1 조합전략 | 10 | 1,000만원 | 15% | 50% | 10% | 10% | 0.74* | 67%* |
| `compose-and1` | 주간 AND-1 조합전략 | 5 | 2,000만원 | 15% | 50% | 10% | 10% | 1.75* | 80%* |
| `compose-score1` | 주간 SCORE-1 조합전략 | 5 | 2,000만원 | 15% | 50% | 10% | 10% | 1.17* | 67%* |
...
- `*` compose 3개 모델은 train/val 분리 없는 backtest 결과 (val_sharpe/val_승률이 아닌 backtest sharpe/승률). 4주 실전 결과 후 별도 최적화 예정.
...
| **모의투자 Exit 체크** | **평일 15:20 KST** | **정규장 마감 직전, Kiwoom 현재가 체크 → 손절/익절/트레일 매도주문** |
| 시간외 단일가 수집 | 평일 16:05 KST | Kiwoom ka10098 API → DB 저장 |
| 추세 단계 | 평일 16:30 KST | 전 종목 Stage 1/2/3 분류 + Telegram |
...
| 강세 후보 | 일요일 20:30 KST | Ichimoku 7조건 전 종목 스캔 + HTML |
...
**고점 이탈 경고 (`⚠️`):** Stage 3 종목에서 외국인·기관 동시 순매도 2일 연속, 또는 윗꼬리 캔들 + 거래량 급증 시 발송.
...
뉴스 신호는 시장이 이미 반영한 후에 생성되는 후행 지표 특성상 방향성 예측 도구로는 유의미하지 않습니다. 시스템의 실제 알파는 추세 단계 + 강세 후보에 있으며, `/backtest` 로 차트 전략 백테스트를 실행할 수 있습니다.

**Q. Kiwoom API 없이도 시스템이 동작하나요?**
네. `KIWOOM_APP_KEY` 없이는 시간외 단일가 수집(16:05 KST)만 비활성화됩니다. `KIWOOM_MOCK_APPKEY` 없이는 모의투자 잡 3개(09:05/15:20/16:40 KST)가 비활성화됩니다. 나머지 기능은 모두 정상입니다.
