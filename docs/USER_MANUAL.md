> 최종 검증 버전: **v0.8.0.0** | 운영 환경: Windows 11 + Python 3.11
...
5. [Telegram 명령어](#5-telegram-명령어)
   - 정보 조회: /status · /signals · /today · /screener · /volume
...
────────────────────────────────────      ───────────────────────
평일 15:40 거래량 일보 전송                /status   — 시스템 상태
평일 16:30 전 종목 Stage 분류              /signals  — 최근 매매 신호
평일 17:00 감시 종목 워치리스트 전송       /screener — 주봉 스크리닝 결과
매일 16:05 시간외 단일가 수집             /backtest2 — 전략 백테스트
매일 20:00 KRX 종목 리스트 갱신           /buy /sell /port /pnl — 거래 기록
일요일 20:00 주간 뉴스 백테스팅
일요일 20:30 주봉 Ichimoku 스크리닝
7분마다 뉴스 수집 + 신호 감지
```

**이 시스템이 하지 않는 것:** 자동 주문 없음. 모든 매매 결정은 직접 합니다.
...
아니요. 모든 매매는 사용자가 직접 증권사 앱에서 실행합니다. 이 시스템은 분석·알림·기록 전용입니다.
...
네. 시간외 단일가 수집(16:05 KST)만 비활성화되고 나머지 기능은 모두 정상입니다.
