...
전체기간 결론(모델A 우위)만으로는 우연한 단일 구간 결과인지 구분할 수 없어, 아래 holdout 분할 검증을 추가로 실시했다.

### 3차 시도 (전종목, 기간 2분할 holdout 검증)

전체기간(2025-01-01~2026-06-17)을 전반(2025-01~09, n=3,399)·후반(2025-10~2026-06, n=5,310, 진짜 holdout — 전반 구간 결과를 본 뒤 사후에 분리한 것이 아니라 데이터 자체가 시간순으로 분리됨)으로 나눠 같은 비교를 두 번 더 실행:

| 구간 | 신호수 | 모델A win_sell Δ | 모델A avg_sell Δ | 모델B win_sell Δ | 모델B avg_sell Δ |
|---|---|---|---|---|---|
| 2025-01~09 (전반) | 3,399 | +0.5pp | **-0.4pp** | +3.7pp | -0.3pp |
| 2025-10~2026-06 (holdout) | 5,310 | +2.9pp | **+0.4pp** | +4.1pp | -0.4pp |
| 전체 (위 2차 결과) | 8,311 | +1.9pp | +0.3pp | +4.0pp | -0.4pp |

**결론 (재정정): 모델A의 우위는 robust하지 않다.** win_sell 개선은 두 구간 모두 양(+)이지만 크기가 +0.5pp→+2.9pp로 6배 차이가 나고, 더 중요하게는 **avg_sell 개선의 부호 자체가 구간마다 바뀐다**(전반 -0.4pp 악화 / holdout +0.4pp 개선). 전체기간의 +0.3pp는 holdout 구간의 개선이 전반 구간의 악화를 상쇄하고 남은 순효과일 뿐이라, "모델A가 평균수익률을 개선한다"는 주장은 이 데이터로 뒷받침되지 않는다.

반면 **모델B(3단계분할)는 두 구간 모두 패턴이 일관된다** — win_sell +3.7~+4.1pp, avg_sell -0.3~-0.4pp로 방향·크기 모두 안정적. "승률은 오르지만 평균수익률은 소폭 낮아진다"는 트레이드오프 자체는 신뢰할 수 있다는 의미이며, 이게 원본보다 "더 좋다"는 뜻은 아니다(목표 함수에 따라 선택 — 승률 중시면 모델B, 수익률 중시면 원본).

**권고 (최종)**: 이번 holdout 검증으로 모델A 전환 근거는 약해졌다 — **현재 운영 중인 원본 모델(`OPTIMAL_EXIT_PARAMS`)을 유지**하고, 모델A/B 모두 `jobs/paper_jobs.py`에 연결하지 않는다. 모델B의 "고승률·저수익" 트레이드오프는 일관되게 재현되므로, 향후 승률을 우선시하는 별도 전략 슬롯이 필요해질 때 후보로 재검토할 수 있다. 두 모델 모두 `OPTIMAL_EXIT_PARAMS`처럼 train/val 그리드서치를 거치지 않았으므로, 재검토 시에도 단일 holdout 분할보다 더 엄격한 walk-forward 검증을 권장한다.
