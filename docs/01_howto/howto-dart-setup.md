...
## 4단계: 3-Pass 컬럼 마이그레이션 (기존 설치만)

기존 `dart_extractions` 테이블이 있는 경우, 3-Pass 구조화 결과 컬럼 3개를 추가합니다.
신규 설치라면 `init_db()`가 자동으로 처리하므로 건너뜁니다.

```bash
python data/dart_extractor.py --migrate-schema
```

출력:
```
마이그레이션 완료: segments_json / revenue_json / competitors_json 컬럼 추가
```

## 5단계: Ollama 추출 실행

다운로드된 XML에서 Ollama로 투자 판단 내러티브와 3-Pass 구조화 JSON을 추출합니다.
...
```
[1/4] XML 추출: reports/dart/삼성전자/20260313000662_사업보고서 (2025.12)/20260313000662.xml
      anchor+grep: 18,432자
[2/4] 서술 추출 (기존)
[3/4] 3-Pass 구조화 추출 (병렬)
[4/4] 결과 (18,432자 컨텍스트)
────────────────────────────────────────────────────────────────────────
[서술 텍스트]
────────────────────────────────────────────────────────────────────────
...
## segments_json [OK]
[{"segment_name": "DS부문", "products": ["HBM", "DRAM", "NAND"], ...}]

## revenue_json [OK]
{"periods": ["2023", "2024"], "segments": [...], "consolidated": {...}, "unit": "백만원"}

## competitors_json [OK]
[{"name": "SK하이닉스", "relation_type": "direct", ...}]
```
...
> **스케줄 타이밍 근거**: 각 분기보고서는 결산월로부터 약 45일 내 제출된다. spring(5/20)은 3월말 제출 사업보고서와 5월 제출 1분기보고서가 모두 완료된 시점, autumn(9/1)은 8월 중 제출되는 반기보고서 완료 후, winter(11/20)은 11월 중 제출되는 3분기보고서 완료 후에 실행된다.
