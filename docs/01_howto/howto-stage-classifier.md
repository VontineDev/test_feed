...
| 축 | 조건 |
|----|------|
| **가격·기술** | 당일 상승률 ≥ +5%(KOSPI) / +7%(KOSDAQ). MA20 > MA20(5일 전). RSI ≥ 50. 52주 고점 대비 괴리율 0~20% 구간. |
| **거래대금·거래량** | 당일 거래대금 ≥ 20일 평균의 200%. 당일 거래량 ≥ 20일 평균의 200%. |
| **수급** | 당일 외국인 순매수 > 0. 기관 순매수 ≥ 0 (동시 순매수 시 가중치 상승). 최근 3거래일 외인+기관 합산 순매수량 ≥ 유통주식수의 0.2%. 또는 기관 3일 이상 연속 순매수 + 외인 순매도 규모 축소·전환. |
...
| 축 | 조건 |
|----|------|
| **가격·기술** | 현재가가 Stage 1 고점 대비 -5%~-20% 구간. MA20 위 또는 MA20의 -5% 이내. 일일 고저폭이 급등일 대비 뚜렷하게 축소. |
| **거래대금·거래량** | 거래대금이 급등일 대비 30~60% 수준으로 감소. 단, 20일 평균 대비 100~150%는 유지. |
| **수급** | 기관: 구간 전체적으로 순매수 유지 또는 누적 보유량 증가 추세. 외국인: 단기 소매도·소매수 반복 가능하나 지분율이 단기간에 1%p 이상 빠지지 않음. 개인: 순매수 감소 또는 순매도 우위 ("개인 출회 + 기관 받는" 패턴이면 재매집 신호). |
...
**전제:** 직전에 Stage 2 조건을 만족했던 종목.

| 축 | 조건 |
|----|------|
| **가격·기술** | 조정 구간 고점 또는 52주 고점 돌파. 일일 상승률 +5~10% 이상(장대양봉 또는 갭상승). RSI(14) ≥ 70. |
| **거래대금·거래량** | 거래대금이 Stage 2 평균 대비 150~200% 재증가. Stage 1 급등일 수준 이상. |
| **수급** | 외국인·기관 동시 순매수. 최근 2~3일 합산 외인+기관 순매수량 ≥ 유통주식수의 0.2%. |
...
| | 외국인·기관 연속 순매수/순매도 일수 (foreign_streak, inst_streak) | 1·2 |
| | 외국인 지분율 변화 — 14일 누적 외국인 순매수 / 상장주식수 (`foreign_chg_14d_pct`) | 2·3 |
| | 수급 컨빅션 점수 (`flow_score`, -1.0 ~ +1.0) | 전체 |
...
- `krx_listings` 테이블 — `listed_shares` 컬럼 (foreign_chg_14d_pct 계산에 필요)
