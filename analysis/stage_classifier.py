...
classify_stage_v15(ticker, price_df, flow_df, s1_history, s2_history, market, listed_shares) -> int | None
    v1.5 (production): v1.4 + 20일 박스권 이탈 조건
    1  =  Stage 1 (랠리 초입)
    2  =  Stage 2 (중간 조정·재매집)
    3  =  Stage 3 (과열 재가속)
    None  =  어느 단계도 해당 없음

버전 체인은 누적 구조 — v15는 _check_stage1_v14→v13→v11→v1.0,
_check_stage2_v13→v12→v11, _check_stage3_v12→v11 조건 헬퍼를 그대로 호출하므로
이 헬퍼들은 여기(라이브 경로)에 남아 있다. 구버전 진입점(classify_stage v1.0,
classify_stage_v11~v14)과 v1.0 전용 헬퍼는 analysis.stage_classifier_legacy로
분리 — 버전별 차이 요약도 그쪽 docstring 참조.
